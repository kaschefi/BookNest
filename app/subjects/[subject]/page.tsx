...
import { subjectStaticData, RichLesson } from "@/lib/subjectData";
...
// Custom type combining MongoDB lesson information with static styling
interface MergedLesson {
    _id: string | null; // MongoDB ID, null if not in DB yet
    name: string;
    description: string;
    index: number;
    slug: string;
        ...
        const fetchLessonResources = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    if (token && data.resources?.length > 0) {
